                          ...
                          model = 'binomial')
{
  ...
  {
    # P(X <= nv) for X~BetaBin(.|coverage, p)
    log_p = sapply(
      nvs,
      VGAM::dbetabinom,
      size = coverage,
  ...
  # For very low purities the cumulative can be always higher than alpha
...
purity_bmix = function(nv, coverage, purity, model, eps) {
  
  inputs = data.frame(nv = nv,
                      coverage = coverage,
                      purity = purity)
  
  if(length(nv) <= 3) return(list(
    data = inputs,
    fit = NA,
    purity = NA,
    plot_bmix = NA
  ...
  if (model == 'binomial'){
    ...
    fit = BMix::bmixfit(data, K.Binomials = 1:3, K.BetaBinomials = 0)
    n_binomials = fit$K["B"]
    ...
    fit = BMix::bmixfit(data, K.Binomials = 0, K.BetaBinomials = 1:3)
  ...
  bmix_best_purity = case_when(
    ...
    n_binomials == 2 & min(abs(peaks[2]*2 - purity), abs(peaks[1]*2 - purity)) <= eps ~ peaks[which.min(c(abs(peaks[2]*2 - purity), abs(peaks[1]*2 - purity)))]*2,
    TRUE ~ NA %>% as.double()
